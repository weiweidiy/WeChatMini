...
namespace TowerDefense.Towers.Placement
{
    ...
    public class TowerPlacementGrid : MonoBehaviour, IPlacementArea
    {
            ...
            if (tileToUse != null)
            {
                ...
                for (int y = 0; y < dimensions.y; y++)
                {
                    for (int x = 0; x < dimensions.x; x++)
                    {
                        ...
                        newTile.transform.localScale *= gridSize;
                        newTile.TowerNumber = GameManager.instance.towerNumber;
                        GameManager.instance.towerNumber++;
        ...
        public (PlacementTile, IntVector2) GetTileByNumber(int number)
        {
            for (int y = 0; y < dimensions.y; y++)
            {
                for (int x = 0; x < dimensions.x; x++)
                {
                    if (m_Tiles[x, y].TowerNumber == number)
                        return (m_Tiles[x, y], new IntVector2(x, y));
                }
            }
            return (null, new IntVector2(-1, -1));
        }
